...
2570-02 Module 1 - IDPS Configuration
Distributed and GWFW
'''
...
class IdpsT0:
  ...
  def __init__(self,t0id):
    self.t0id = t0id

  def enable_idps(self):
    data = {'resource_type':'Infra', 'children':[{'resource_type':'ChildResourceReference', 'id':'{}'.format(self.t0id), 'target_type':'Tier0', 'children':[{'Tier0SecurityFeatures':{'resource_type':'Tier0SecurityFeatures', 'features':[{'feature':'IDPS', 'enable':'true'}, {'feature':'IDFW', 'enable':'false'}]}, 'resource_type':'ChildTier0SecurityFeatures'}]}]}   
...
print('Enabling IDPS on T0 and T1s')

for tier0 in t0list:
  print('Configuring Tier-0 {}'.format(tier0))
  tier0gw = IdpsT0(tier0)
  tier0gw.enable_idps()
  time.sleep(3)
